...
import { AlertCircle, ArrowRight, Check, Database, FileSpreadsheet, UploadCloud } from "lucide-react";
...
export default function ImportPage() {
  ...
  const readFile = (file?: File) => {
    ...
    reader.onload = () => { const rows = String(reader.result).split(/\r?\n/).filter(Boolean).map((line) => line.split(",").map((value) => value.trim())); if (!rows[0]?.some((value) => /name/i.test(value))) { setFileError("The CSV needs a name or full_name column."); return; } setFileRows(rows); setFileError(undefined); setState("preview"); pagination.resetPage(); notify("CSV validated — preview ready"); };
  ...
  const handleDrop = (event: DragEvent<HTMLLabelElement>) => { event.preventDefault(); setDragging(false); readFile(event.dataTransfer.files?.[0]); };

  return <div className="page import-page"><PageHeader eyebrow="Data intake" title="Import relationship data" description="Preview, validate and confirm records before anything joins the clean relationship graph." action={<Badge tone="neutral">Local demo repository</Badge>} />
    <div className="import-layout">
      <section className="card import-source" data-spotlight><div className="source-top"><div className="source-icon"><Database size={22} /></div><Badge tone="ai">Demo source</Badge></div><div><h2>Demo dataset</h2><p>72 fictional records with realistic inconsistencies, duplicates and missing fields.</p><div className="database-visual" aria-hidden="true"><i /><i /><i /><span>Structured relationship records</span></div><div className="tags"><span className="tag">72 records</span><span className="tag">20 fields</span><span className="tag accent">Synthetic data</span></div></div><button className="button button-primary" onClick={chooseDemo} data-testid="load-demo"><Database size={16} /> Load demo data</button></section>
      <div className="or-divider"><span>or</span></div>
      <label className={`upload-zone ${dragging ? "is-dragging" : ""}`} onDragEnter={(event) => { event.preventDefault(); setDragging(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={() => setDragging(false)} onDrop={handleDrop}><input type="file" accept=".csv,text/csv" onChange={handleFile} /><span className="upload-icon"><FileSpreadsheet size={24} /></span><strong>CSV import</strong><p>Drop a CSV here or choose a file</p><small>UTF-8 CSV · up to 1 MB · headers required</small><span className="upload-action"><UploadCloud size={14} /> Choose file</span></label>
      {fileError && <div className="form-error"><AlertCircle size={16} />{fileError}</div>}
      {state !== "ready" && <section className="card import-preview"><div className="card-header"><div><p className="eyebrow">Validation preview</p><h2>{fileRows ? "Uploaded CSV" : "Demo dataset"}</h2></div><Badge tone="good"><Check size={12} /> Ready to import</Badge></div><div className="validation-grid"><div><small>Rows detected</small><strong>{previewRows.length}</strong></div><div><small>Fields mapped</small><strong>{fileRows?.[0]?.length || 20}</strong></div><div><small>Warnings expected</small><strong>{fileRows ? "Review" : "23"}</strong></div><div><small>Blocking errors</small><strong>0</strong></div></div><div className="field-summary"><h3>Processing preview</h3><div><span><Check /> Trim and normalize text</span><span><Check /> Validate contact details</span><span><Check /> Flag duplicate candidates</span><span><Check /> Classify profiles</span><span><Check /> Calculate fit and matches</span></div></div><div className="preview-table"><table><thead><tr>{headers.slice(0, 5).map((heading) => <th key={heading}>{heading}</th>)}</tr></thead><tbody>{pagination.items.map((row, index) => <tr key={`${pagination.startIndex + index}-${row.join("|")}`} data-testid="import-preview-row">{row.slice(0, 5).map((cell, column) => <td key={`${column}-${cell}`}>{cell}</td>)}</tr>)}</tbody></table></div><Pagination page={pagination.page} pageSize={pagination.pageSize} totalItems={pagination.totalItems} totalPages={pagination.totalPages} onPageChange={pagination.setPage} itemLabel="import rows" /><div className="confirm-bar"><p>{fileRows ? "This prototype validates uploaded CSVs; confirmed processing uses the stable seeded dataset." : "Confirm to replace the current demo view and run the full processing pipeline."}</p>{state === "done" ? <span className="success-message"><Check size={16} /> Import complete</span> : <button className="button button-primary" onClick={() => void confirm()} disabled={state === "importing"}>{state === "importing" ? "Processing…" : <>Confirm and process <ArrowRight size={16} /></>}</button>}</div></section>}
    </div>
  </div>;
}
